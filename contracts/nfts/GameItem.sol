...
contract GameItem is AccessControlEnumerable, ERC721Enumerable, ERC721Burnable  {
    ...
    function mint(uint256 _tokenId, bytes memory _signature) public virtual {
        require(!nonces[_tokenId], "GameItem: nonce was used");

        address msgSender = _msgSender();

        bytes32 message = keccak256(abi.encodePacked(_tokenId, msgSender, chainId, this)).prefixed();

        require(hasRole(SIGNER_ROLE, message.recoverSigner(_signature)), "GameItem: signature is invalid");
    ...
    function mintBatch(uint256[] memory _tokenIds, address[] memory _accounts) public virtual {
        ...
        require(length > 0 && length == _accounts.length, "GameItem: array length is invalid");
    ...
    function safeTransferBatch(address _from, address[] memory _receivers, uint256[] memory _tokenIds) public virtual {
        ...
        require(length > 0 && length == _tokenIds.length, "GameItem: array length is invalid");
